...
class JobsUpsSpider(scrapy.Spider):
    ...
    def parse_receiverTemplateJSON(self, response):
        ...
        for sel in response.xpath('//*[@id="content"]'):
            ...
            for sel1 in response.xpath('//*[@id="description"]'):
                ...
                tempUrl = response.follow(tempUrl, self.parse_test)
                receiverTemplateJSON['address'] = tempUrl
    ...
    def parse_test(self, response):
        for result in response.xpath('//*[@id="content"]/div[1]/section/h2'):
            testOut = result.css('span::text').extract_first()
            yield testOut
